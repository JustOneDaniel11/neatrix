...
import { CleaningLoader } from '../ui/CleaningLoader';
    ...
    return (
      <div className="bg-white rounded-lg p-6 border border-gray-200">
        <div className="flex items-center justify-center py-8">
          <CleaningLoader size="md" text="Loading tracking information..." />
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden shadow-lg">
      ...
      <div className="bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-gray-800 dark:to-gray-700 p-4 sm:p-6">
        ...
        {/* Current Stage Info */}
        <div className="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-100 dark:border-gray-600 mb-8 shadow-sm relative z-0">
        ...
        <div className="w-full max-w-full overflow-hidden">
          ...
          <div className="hidden sm:block">
            <div className="relative px-6 py-8 z-10">
              {/* Progress Line Background - Perfectly centered with circles, avoiding overlap */}
              <div className="absolute h-1 bg-gray-200 dark:bg-gray-700 rounded-full" style={{ 
                top: '50%', 
              ...
              {/* Animated Progress Line with Gradient - Perfectly centered with circles, avoiding overlap */}
              <div 
                className="absolute h-1 bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 rounded-full transition-all duration-1000 ease-out shadow-lg"
                style={{ 
                  top: '50%',
                  transform: 'translateY(-50%)',
                  left: 'calc(3rem + 1.5rem)',
                  width: `calc(${Math.max(0, (progressPercentage / 100) * (trackingStages.length > 1 ? ((trackingStages.length - 1) / trackingStages.length) * 100 : 0))}% - 3rem)`,
              ...
              <div className="grid grid-cols-auto gap-0 relative" style={{ gridTemplateColumns: `repeat(${trackingStages.length}, 1fr)` }}>
                {trackingStages.map((stage, index) => {
                  ...
                  return (
                    <div key={stage.stage_name} className="flex flex-col items-center group relative z-20">
                      {/* Stage Node with Enhanced Styling - Responsive */}
                       <div className={`
                         relative w-10 h-10 sm:w-12 sm:h-12 rounded-full border-2 flex items-center justify-center transition-all duration-500 transform hover:scale-110 focus:scale-110 cursor-pointer z-30 touch-manipulation
                         ${isCompleted 
                      ...
                      {/* Enhanced Tooltip with Stage Descriptions - Responsive */}
                      <div className="absolute bottom-full mb-3 left-1/2 transform -translate-x-1/2 bg-gray-900 dark:bg-gray-800 text-white text-xs sm:text-sm rounded-xl px-3 sm:px-4 py-2 sm:py-3 opacity-0 group-hover:opacity-100 group-focus:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-[9999] shadow-2xl border border-gray-700 min-w-max max-w-xs sm:max-w-sm">
                  ...
                  {trackingStages.map((stage, index) => {
                    ...
                    return (
                      <div key={stage.stage_name} className="flex flex-col items-center group relative">
                        ...
                        {/* Connection Line for Mobile - Perfectly centered, avoiding overlap */}
                        {index < trackingStages.length - 1 && (
                          <div className={`absolute w-6 h-0.5 transition-colors duration-500 ${
                        ...
                        {/* Mobile Tooltip - Touch-friendly */}
                        <div className="absolute bottom-full mb-3 left-1/2 transform -translate-x-1/2 bg-gray-900 dark:bg-gray-800 text-white text-xs rounded-xl px-3 py-2 opacity-0 group-hover:opacity-100 group-active:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-[9999] shadow-2xl border border-gray-700 min-w-max max-w-xs">
                          <div className="text-center">
                            <div className="font-semibold text-white">{stage.stage_label}</div>
                            <div className="text-gray-300 mt-1">
                              {stage.stage_name === 'intake' && 'Items received and logged'}
