  ...
  FOR tbl IN SELECT unnest(ARRAY[
    ...
    IF EXISTS (
      ...
      IF NOT EXISTS (
        ...
        WHERE schemaname = 'public' AND tablename = tbl AND policyname = 'dev_read_all'
      ) THEN
...
BEGIN
  FOR tbl IN SELECT unnest(ARRAY[
    ...
    IF EXISTS (
      ...
      IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' AND tablename = tbl AND policyname = 'dev_write_authenticated'
      ) THEN
        EXECUTE format('CREATE POLICY "dev_write_authenticated" ON public.%I FOR INSERT TO authenticated WITH CHECK (true);', tbl);
      END IF;
      IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE schemaname = 'public' AND tablename = tbl AND policyname = 'dev_update_authenticated'
      ) THEN
...
-- SELECT format('DROP POLICY "%s" ON public.%s;', policyname, tablename)
-- FROM pg_policies WHERE schemaname='public' AND policyname LIKE 'dev_%';